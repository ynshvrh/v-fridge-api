...
public sealed class VChefClient(HttpClient http, ILogger<VChefClient> logger) : IVChefClient
{
    ...
    public async Task<VChefMealPlanResponse?> GenerateMealPlanAsync(VChefMealPlanRequest request, CancellationToken ct = default)
    {
        try
        {
            var response = await http.PostAsJsonAsync("/api/v1/mealplan/generate", request, ct);
            if (!response.IsSuccessStatusCode)
            {
                var errContent = await response.Content.ReadAsStringAsync(ct);
                logger.LogWarning("V-Chef microservice returned status {Status}: {Error}", response.StatusCode, errContent);
            ...
            return await response.Content.ReadFromJsonAsync<VChefMealPlanResponse>(cancellationToken: ct);
        }
        catch (Exception ex)
        {
